...
* Check the `ConnectionStrings` in `appsettings.json` files under the `devset_front_end.HttpApi.Host` and `devset_front_end.DbMigrator` projects and change it if you need.
...
* Run `devset_front_end.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p c24580e6-9a18-4448-9f45-54368d6d56b6
```

> `c24580e6-9a18-4448-9f45-54368d6d56b6` is the password of the certificate, you can change it to any password you want.
...
* `devset_front_end.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `devset_front_end.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
